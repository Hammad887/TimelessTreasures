...
const EditStory = ({ onClose, storyToEdit, actuallyEdited }) => {
    ...
    const onSubmit = async () => {
        try {
            ...
            const storyCode = storyToEdit.storyCode;

            const storyData = {
                storyCode: storyCode,
                user: storyToEdit.user,
                title: title,
                synopsis: synopsis,
                mainImage: mainIMG,
    ...
    const [ancestry, setAncestry] = useState(storyToEdit.ancestryTag);
    const [origins, setOrigins] = useState(storyToEdit.originsTag);
    const [types, setTypes] = useState(storyToEdit.typesTag);
    const [originOptions, setOriginOptions] = useState(originOptionsSource);

    const [privacy, setPrivacy] = useState(storyToEdit.view);
    ...
    const [synopsis, setSynopsis] = useState(storyToEdit.synopsis);
    const [story, setStory] = useState(storyToEdit.story);

    const [imageSrc, setImageSrc] = useState(null);
    const [mainImage, setMainImage] = useState(null);
    const [mainImageURL, setMainImageURL] = useState(storyToEdit.mainImage);
        ...
        <ModalContent maxW={{ base: "95%", lg: "80%" }}>
            ...
            {(screen === 1) ?
                ...
                <Box>
                    ...
                    <ModalHeader fontWeight={"bold"} fontSize={"2xl"} pb={'0px'} textAlign="center">Edit Your Story</ModalHeader>
