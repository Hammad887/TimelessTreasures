...
import PostCard from './StoryPostCard.js';
...
const StoryPostCardView = ({ addingCheck, filters }) => {
    ...
    const fetchStories = async (lastVisibleDoc) => {
        if (!loading) {
            ...
            if (lastVisibleDoc) {
                q = query(storiesRef, orderBy('timestamp', 'desc'), startAfter(lastVisibleDoc), limit(paginationSize));
            } else {
                q = query(storiesRef, orderBy('timestamp', 'desc'), limit(paginationSize));
            }
    ...
    useEffect(() => {
        ...
        if (stories && filters && (filters.origins.length > 0 || filters.ancestry != '' || filters.types.length > 0)) {
            const filtering = stories.filter((story) => {
                ...
                if (filters.ancestry && filters.ancestry.length > 0 && !filters.ancestry.some((ancestry) => story.ancestryTag.includes(ancestry))) {
                    return false;
                }
...
export default StoryPostCardView;