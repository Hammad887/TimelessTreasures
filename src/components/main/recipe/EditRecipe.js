...
const EditRecipe = ({ onClose, recipeToEdit, actuallyEdited }) => {
    ...
    const onSubmit = async () => {
        try {
            ...
            const recipeCode = recipeToEdit.recipeCode;

            const recipeData = {
                recipeCode: recipeCode,
                user: recipeToEdit.user,
                title: title,
    ...
    const [ancestry, setAncestry] = useState(recipeToEdit.ancestryTag);
    const [origins, setOrigins] = useState(recipeToEdit.originsTag);
    const [types, setTypes] = useState(recipeToEdit.typesTag);
    const [originOptions, setOriginOptions] = useState(originOptionsSource);

    const [privacy, setPrivacy] = useState(recipeToEdit.view);
    ...
    const [story, setStory] = useState(recipeToEdit.story);

    const [imageSrc, setImageSrc] = useState(null);
    const [mainImage, setMainImage] = useState(null);
    const [mainImageURL, setMainImageURL] = useState(recipeToEdit.mainImage);
        ...
        <ModalContent maxW={{ base: "95%", lg: "80%" }}>
            ...
            {(screen === 1) ?
                ...
                <Box>
                    ...
                    <ModalHeader fontWeight={"bold"} fontSize={"2xl"} pb={'0px'} textAlign="center">Edit Your Recipe</ModalHeader>
